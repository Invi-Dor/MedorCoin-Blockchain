...
, msgHandler_(std::make_shared<MessageHandler>())
...
tcp::endpoint ep(tcp::v4(),
    static_cast<uint16_t>(cfg_.listenPort));
...
size_t threads = cfg_.ioThreads > 0
    ? cfg_.ioThreads
    : std::max(size_t(1),
...
slog(0, "[P2PNode] Listening on port "
...
bool P2PNode::isRunning() const noexcept {
...
registry_->broadcast(std::move(frame));
}
...
if (!g_reconnect.shouldRetry(key, cfg_.maxReconnectAttempts))
return;
...
peerMgr_->removePeer(peerId);
}

void P2PNode::slog(int level, const std::string& msg) {
std::lock_guard<std::mutex> lk(callbackMu_);
if (logFn_) {
try { logFn_(level, msg); } catch (…) {}
} else {
std::cout << msg << “\n”;
}
}
...
```
    ...
    if (!cfg_.tlsCAFile.empty()) {
        sslCtx_.load_verify_file(cfg_.tlsCAFile);
...
acceptor_.async_accept(
    ...
    [this, session](const boost::system::error_code& ec) {
        if (!running_.load()) return;
        if (!ec) {
            if (peerMgr_->peerCount() < cfg_.maxInboundPeers) {
...
registry_->add(peerId, registry_->get(peerId));
std::lock_guard<std::mutex> lk(callbackMu_);
if (peerConnFn_) {
