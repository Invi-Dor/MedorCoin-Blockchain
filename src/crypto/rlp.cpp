...
// pad32, encodeBytes/encodeUInt/encodeList left as-is from your original
static void encodeLength(size_t len, uint8_t offset, std::vector<uint8_t>& out) {
  if (len < 56) out.push_back(uint8_t(offset + len));
  else {
    std::vector<uint8_t> tmp;
    size_t t = len;
    while (t > 0) { tmp.push_back(uint8_t(t & 0xff)); t >>= 8; }
...
std::vector<uint8_t> encodeUInt(uint64_t value) {
  ...
  while (value > 0) { buf.push_back(uint8_t(value & 0xff)); value >>= 8; }
...
std::vector<uint8_t> encodeList(const std::vector<std::vector<uint8_t>>& items) {
  ...
  for (auto const& i : items) payload.insert(payload.end(), i.begin(), i.end());
  std::vector<uint8_t> out;
