...
inline uint64_t rotl64(uint64_t x, unsigned int n) noexcept {
...
// Implementation (inline for header-only use)

inline Keccak256::Keccak256() noexcept
    : state_(), rate_(136), pos_(0) {
    reset();
}

inline void Keccak256::reset() noexcept {
    std::memset(state_.data(), 0, state_.size());
    pos_ = 0;
}

inline void Keccak256::update(const uint8_t* data, size_t length) noexcept {
    size_t i = 0;
    while (length > 0) {
...
inline std::array<uint8_t, Keccak256::HASH_SIZE> Keccak256::digest() noexcept {
...
inline void Keccak256::keccakf1600(std::array<uint8_t, 200>& state) noexcept {
