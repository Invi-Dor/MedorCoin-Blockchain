...
bool verify_hash(
    ...
    if (secp256k1_ecdsa_recoverable_signature_parse_compact(
            ctx, &recsig, sig65, (int)sig65[64]) != 1) {
        secp256k1_context_destroy(ctx);
        return false;
    }
    ...
    int ok = secp256k1_ecdsa_verify(
        ctx, &normsig, hash32, &pubkey);
